...
## Create matrix
newmods.matrix.ur.low <- matrix(0, nrow = nrow(data.ur.low), ncol = 10)
...
mreg.ur.low.pred <- predict(mreg.ur.low %>%
  ...
  mutate(x = as.numeric(rownames(.)),
         ...
         ur_cat = "Low to moderate [3.1%, 6.0%)")
...
## Create matrix
newmods.matrix.ur.high <- matrix(0, nrow = nrow(data.ur.high), ncol = 7)
...
mreg.ur.high.pred <- predict(mreg.ur.high %>%
  ...
  mutate(x = as.numeric(rownames(.)),
         ...
         ur_cat = "Moderate to high [6.0%, 10.4%]")
