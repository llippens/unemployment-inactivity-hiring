...
## Create matrix
newmods.matrix.ur.low <- matrix(0, nrow = nrow(data.ur.low), ncol = 8)
...
## Create matrix
newmods.matrix.ur.high <- matrix(0, nrow = nrow(data.ur.high), ncol = 8)

colnames(newmods.matrix.ur.high) <- c(
  "unemployment_duration_months",
  "response_typePositive reaction",
  "regionEurope (Western)", "regionEurope (Northern)",
  "years_average",
  "genderFemale",
  "age_categoricalYoung (21-30)", "age_categoricalUnknown"
)
...
newmods.matrix.ur.high[, "regionEurope (Northern)"] <- ctable.ur.high %>% filter(region == "Europe (Northern)") %>% pull()

newmods.matrix.ur.high[, "genderFemale"] <- gtable.ur.high %>% filter(gender == "Female") %>% pull()

newmods.matrix.ur.high[, "age_categoricalYoung (21-30)"] <- atable.ur.high %>% filter(acat == "Young (21-30)") %>% pull()
newmods.matrix.ur.high[, "age_categoricalUnknown"] <- atable.ur.high %>% filter(acat == "Unknown") %>% pull()
